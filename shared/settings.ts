...
export const personaSchema = z.object({
  ...
  provider: z.string().trim().regex(/^[a-zA-Z0-9_-]+$/, "Choose a valid provider.").max(120, "Choose a valid provider.").default("codex")
    .refine((id) => !["auto-mode-for-paseo", "auto-jev-codex-for-paseo"].includes(id), "A persona cannot route back to Auto Mode."),
...
}).refine((persona) => persona.taskTypesAuto || persona.taskTypes.length > 0,
...
const PRESET_TASK_TYPES: Record<string, (typeof TASK_TYPES)[number]> = {
...
].map((persona) => ({ ...persona, workMode: "", taskDepth:
  persona.id === "reporter" ? "medium" as const : persona.id === "writer" ? "high" as const : "xhigh" as const,
  taskTypes: [PRESET_TASK_TYPES[persona.id]], taskTypesAuto: true, taskTypesScope: persona.description }));

export const MAX_PERSONAS = 32;

const personasSchema = z.array(personaSchema).max(MAX_PERSONAS, `Use at most ${MAX_PERSONAS} personas.`).superRefine((personas, context) => {
  const ids = new Set<string>();
  for (const [index, persona] of personas.entries()) {
    if (ids.has(persona.id)) context.addIssue({ code: "custom", path: [index, "id"], message: "Persona IDs must be unique." });
    ids.add(persona.id);
  }
});

export const settingsSchema = z.object({
  personaScopeVersion: z.literal(1).default(1),
  ...
  personas: personasSchema.default(defaultPersonas),
});
